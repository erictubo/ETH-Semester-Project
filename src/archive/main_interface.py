...
"""
Create new processed railway object (and save to file) or load existing railway object from file.
"""
...
for i, keyframe in enumerate(keyframes):
    ...
    for track in local_tracks:
        ...
        for i in range(len(points_cam)-2, 0, -1):
            point = points_cam[i]
            if np.arccos(np.dot(point, previous_point) / (np.linalg.norm(point) * np.linalg.norm(previous_point))) < 0.004:
                points_cam.pop(i)
            else:
...
print("Pose vector [t, q]:", keyframe.Camera.pose_vector)
print("H_gps_cam: R, t", keyframe.Camera.H_gps_cam[0:3, 0:3], keyframe.Camera.H_gps_cam[0:3, 3])


# Evaluate final pose on all keyframes
# Use final estimate to reproject points on all images